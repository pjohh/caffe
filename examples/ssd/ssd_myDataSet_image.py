...
# load labels
voc_labelmap_file = 'data/myDataSet/labelmap.prototxt'
...
for ssd_size in ['SSD_300x300', 'SSD_500x500']:
    snapshot_dir = "models/VGGNet/myDataSet/{}".format(ssd_size)
    ...
    for file in os.listdir(snapshot_dir):
      if file.endswith(".caffemodel"):
        ...
        iter = int(basename.split("VGG_myDataSet_{}_iter_".format(ssd_size))[1])
        if iter > max_iter:
          max_iter = iter
    ...
    # load model
    model_def = 'models/VGGNet/myDataSet/{}/deploy.prototxt'.format(ssd_size)
    model_weights = 'models/VGGNet/myDataSet/{}/VGG_myDataSet_{}_iter_{}.caffemodel'.format(ssd_size, ssd_size, max_iter)
    ...
    plt.figure()
    plt.title('VGG_myDataSet_{}'.format(ssd_size))
    currentAxis = plt.gca()
    plt.imshow(image)
    plt.axis('off')
